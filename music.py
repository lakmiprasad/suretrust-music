...
class player(tk.Frame):
    ...
    def track_widgets(self):
        ...
        self.song_track = tk.Label(self.track, font=("times new roman", 15 , "bold"),bg='#FFFFFF',fg='dark blue' ) 
        self.song_track['text'] = 'My Musicplayer'
    ...
    def control_widgets(self):
        ...
        self.prev = tk.Button(self.controls, image= prev,width=50,height=80)
        self.prev['command'] = self.prev_song
        self.prev.grid(row=0,column=1 )

        self.pause = tk.Button(self.controls,image= pause ,width=50,height=80)
        self.pause['command'] = self.pause_song
        self.pause.grid(row=0,column=2 )

        self.next = tk.Button(self.controls, image= next_,width=70,height=80)
    ...
    def retrieve_song(self):
        ...
        for wind_,dirs, files in os.walk(directory):
            for file in files:
                if os.path.splitext(file)[1] == '.mp3':
                    path= (wind_+ '/' +file).replace('\\','/')
        ...
        self.tracklist['text'] = text=f'playlist - {len(self.playlist)}'
    ...
    def play_songs(self,event=None):
        ...
        self.song_track['text'] = os.path.basename(self.playlist[self.current])
